...
USaveMapInstance::FOnPreSaveWorld		USaveMapInstance::PreSaveWorld;
USaveMapInstance::FOnPostSaveWorld		USaveMapInstance::PostSaveWorld;
...
void USaveMapInstance::TestSave()
{
...
bool USaveMapInstance::Exec(UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Out)
{
...
bool USaveMapInstance::Exec_Obj(const TCHAR* Str, FOutputDevice& Ar)
{
...
bool USaveMapInstance::SaveCurrentLevel()
{
	...
	if (WorldToSave)
	{
		FString ForcePath = GetAutoSaveDir();
		FString FinalFilename;
		bReturnCode = SaveWorld(WorldToSave, &ForcePath,
			NULL, NULL,
			true, false,
			FinalFilename,
			false, false);
...
bool USaveMapInstance::SaveMap(UWorld* InWorld, const FString& Filename)
{
...
bool USaveMapInstance::SaveAsImplementation(UWorld* InWorld, const FString& DefaultFilename, const bool bAllowStreamingLevelRename, FString* OutSavedFilename)
{
...
bool USaveMapInstance::SaveWorld(UWorld* World,
...
FSavePackageResultStruct USaveMapInstance::Save(UPackage* InOuter, UObject* InBase, EObjectFlags TopLevelFlags, const TCHAR* Filename, FOutputDevice* Error /* = GError */, FLinkerNull* Conform /* = NULL */, bool bForceByteSwapping /* = false */, bool bWarnOfLongFilename /* = true */, uint32 SaveFlags /* = SAVE_None */, const class ITargetPlatform* TargetPlatform /* = NULL */, const FDateTime& FinalTimeStamp /* = FDateTime::MinValue() */, bool bSlowTask /* = true */, class FArchiveDiffMap* InOutDiffMap /* = nullptr */, FSavePackageContext* SavePackageContext /* = nullptr */)
{
...
void USaveMapInstance::OnPreSaveWorld(uint32 SaveFlags, UWorld* World)
{
	if (!ensure(World))
	{
		return;
	...
	USaveMapInstance::PreSaveWorld.Broadcast(SaveFlags, World);
...
void USaveMapInstance::OnPostSaveWorld(uint32 SaveFlags, UWorld* World, uint32 OriginalPackageFlags, bool bSuccess)
{
	if (!ensure(World))
	{
		return;
	...
	USaveMapInstance::PostSaveWorld.Broadcast(SaveFlags, World, bSuccess);
}
bool USaveMapInstance::CanSavePackage(UPackage* PackageToSave)
{
	bool bTemRes = true;
...
bool USaveMapInstance::InitializePhysicsSceneForSaveIfNecessary(UWorld* World, bool &bOutForceInitialized)
{
...
void USaveMapInstance::CleanupPhysicsSceneThatWasInitializedForSave(UWorld* World, bool bForceInitialized)
{
...
UWorld::InitializationValues USaveMapInstance::GetEditorWorldInitializationValues() const
{
	return UWorld::InitializationValues()
		.ShouldSimulatePhysics(false)
		.EnableTraceCollision(true);
}
bool USaveMapInstance::DoesPackageExist(UPackage* Package, FString* OutFilename /* = nullptr */)
{
...
FString USaveMapInstance::GetAutoSaveDir()
{
	//return FPaths::ProjectSavedDir() / TEXT("Autosaves")/TEXT("GameMap") + FPackageName::GetMapPackageExtension();
	return FPaths::ProjectContentDir() / TEXT("GameMap") + FPackageName::GetMapPackageExtension();
}